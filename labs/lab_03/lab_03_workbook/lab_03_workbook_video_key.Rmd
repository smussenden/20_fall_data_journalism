...
```{r}
# filter for places that contain the word city
test_filter <- covid_county_monthly_case_rate %>%
  filter(str_detect(state, "Maryland|Virginia"))

# display in markdown notebook
test_filter
...
test_mutate <- covid_county_monthly_case_rate %>%
  filter(str_detect(county, "city")) %>%
  mutate(city_name = str_remove(county, "city")) %>%
  select(county, city_name, state, everything())
