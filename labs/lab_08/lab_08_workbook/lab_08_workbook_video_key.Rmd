...
# What flaws did you see? Write them here. 



```

Now, I'll walk you through what I saw.  See if you picked out everything I did.  
...
To further shorten them, we can use the function rename(), a version of mutate(), to further shorten them. 
...
**Task (Edit and Run Code): **: run the code below to rename "opioid_prescriptions_100_persons2_2018" to "opioid_prescription_rate".
...
```
Now let's remove row 40, which contained "*Not included - Did not meet inclusion criteria" for each column. We'll use a filter to do this, telling it to return all rows that DO NOT match the text we want gone, using != to do so.  
