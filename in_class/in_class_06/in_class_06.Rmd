...
The catch: the information we need to answer these questions are stored in two separate tables.  

And we have to put them together. In the real world, it's pretty rare that we'll encounter a single magical data source that contains all of the information we need to answer our questions.  

Data journalists call this process of putting together two unrelated tables together to find something new "an enterprise join." As we'll read over the next few weeks, enterprise joins are at the heart of some truly notable works of investigative data journalism.

The example we'll do today is simple, but we're laying the foundation for more complex work.
